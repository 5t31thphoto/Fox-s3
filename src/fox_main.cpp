...
// proves the full product surface first. The COMMANDS[] grammar and do_action()
// dispatch below are already wired; Part 2 only adds the recognizer feeding them.
...
// (init_speech / MultiNet AFE / recognize_offline live in PART 2.)

#include "fox_converse.inc"   // conversation engine (Furby + tiny-LLM brain)
...
static void process_utterance(int16_t* audio, size_t n) {
    uint32_t voiced_ms = fox_voiced_ms(audio, n);   // simple VAD over the buffer

    // ===================== PART 2 PLUGS IN HERE ============================
    // Uncomment once esp-sr is added (see PART2_INTEGRATION.md). It returns a
    // matched command id (>=0) and/or fills `heard` with caught keywords. The
    // dispatch + conversation engine below are already wired for both.
    //
    //   String heard;
    //   int id = recognize_offline(audio, n, heard);
    //   if (id >= 0) {
    //       for (size_t i = 0; i < COMMAND_COUNT; ++i)
    //           if (COMMANDS[i].id == id) { do_action(COMMANDS[i].action); return; }
    //   }
    //   fox_respond(heard, voiced_ms);   // heard may be "" — engine still copes
    //   return;
    // ======================================================================

    // PART 1: no recognizer yet — respond to the SHAPE of your speech via the
    // same conversation engine Part 2 will call (just with heard == "").
    fox_respond("", voiced_ms);
}
...
void setup() {
    ...
    c.internal_mic = false;   // EchoBase mic (proven avatar path)
    ...
    // PART 2 initialises MultiNet speech-to-intent here (see PART2_INTEGRATION.md).
